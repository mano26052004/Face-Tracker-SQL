...
class Face_Recognizer:
    ...
    last_check_time = {}
    def attendance(self, name,student_reg_no,student_dept_code,student_degree_code,student_current_sem_no,student_batch_no,Session):
        current_date = datetime.datetime.now().strftime('%Y-%m-%d')
        conn = sqlite3.connect("attendance.db")
        cursor = conn.cursor()

        # Check if the name is in the last_check_time dictionary
        if name in self.last_check_time:
            last_time = self.last_check_time[name]
            current_time = time.time()
            time_elapsed = current_time - last_time

            # Check if the time elapsed is less than the cooldown duration
            if time_elapsed < self.cooldown_duration:
                print(f"{name} cannot check in or out too quickly. Cooldown is in effect.")
                conn.close()
                return
        ...
        if existing_entry:
            ...
            if existing_entry[3] is None:  # Check if the "out time" is already None
                cursor.execute("UPDATE attendance SET out_time = ? WHERE name = ? AND date = ?",
                               (current_time, name, current_date))
                conn.commit()
                print(f"{name} checked out for {current_date} at {current_time}")
            else:
                print(f"{name} has already checked out for {current_date} at {current_time}")
        else:
            print('from insert area',name,student_reg_no,student_dept_code,student_degree_code,student_current_sem_no,student_batch_no,Session)
            current_time = datetime.datetime.now().strftime('%H:%M:%S')
            cursor.execute("INSERT INTO attendance (name,student_reg_no,student_dept_code,student_degree_code,student_current_sem_no,student_batch_no,in_time, date,Session) VALUES (?, ?, ? , ? ,?, ?, ?, ?,?)",
                           (name,student_reg_no,student_dept_code,student_degree_code,student_current_sem_no,student_batch_no,current_time, current_date,Session))
            conn.commit()
